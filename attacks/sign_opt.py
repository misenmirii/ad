...
class OPT_attack_sign_SGD(object):
    def __init__(self, model, device='cpu', k=200, train_dataset=None):
        self.model = model
        self.device = device
    ...
    def attack_untargeted(self, x0, y0, alpha=0.2, beta=0.001, iterations=1000, query_limit=20000,
        ...
        for i in range(num_directions):
            ...
            if self.predict(x0+torch.tensor(theta, dtype=torch.float).to(self.device)) != y0:
        ...
        if distortion is None or gg < distortion:
            target = self.predict(x0 + torch.tensor(gg*xg, dtype=torch.float).to(self.device))
            print("Succeed distortion {:.4f} target"
                  " {:d} queries {:d} LS queries {:d}\n".format(gg, target, query_count, ls_total))
            return x0 + torch.tensor(gg*xg, dtype=torch.float).to(self.device), gg, True, query_count, xg
        ...
        return x0 + torch.tensor(gg*xg, dtype=torch.float).to(self.device), gg, False, query_count, xg
    ...
    def sign_grad_v1(self, x0, y0, theta, initial_lbd, h=0.001, D=4, target=None):
        ...
        for iii in range(K): # for each u
            ...
            if (target is not None and 
                self.predict(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float).to(self.device)) == target):
                sign = -1

            # Untargeted case
            # preds.append(self.predict(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float).to(self.device)).item())
            if (target is None and
                self.predict(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float).to(self.device)) != y0): # success
                sign = -1
    ...
    def sign_grad_v2(self, x0, y0, theta, initial_lbd, h=0.001, K=200):
        ...
        for _ in range(K):
            ...
            new_theta = theta + h*u
            new_theta /= LA.norm(new_theta)
            if self.predict(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float).to(self.device)) == y0:
                ss = 1
            queries += 1
            sign_grad += sign(u)*ss
        sign_grad /= K
    ...
    def sign_grad_svm(self, x0, y0, theta, initial_lbd, h=0.001, K=100, lr=5.0, target=None):
        ...
        for iii in range(K):
            ...
            # Untargeted case
            if (target is None and
                self.predict(x0+torch.tensor(initial_lbd*new_theta, dtype=torch.float).to(self.device)) != y0):
                sign = -1
    ...
    def fine_grained_binary_search_local(self, model, x0, y0, theta, initial_lbd = 1.0, tol=1e-5):
        ...
        if self.predict(x0+torch.tensor(lbd*theta, dtype=torch.float).to(self.device)) == y0:
            lbd_lo = lbd
            lbd_hi = lbd*1.01
            nquery += 1
            while self.predict(x0+torch.tensor(lbd_hi*theta, dtype=torch.float).to(self.device)) == y0:
                lbd_hi = lbd_hi*1.01
                nquery += 1
                if lbd_hi > 20:
            ...
            lbd_lo = lbd*0.99
            nquery += 1
            while self.predict(x0+torch.tensor(lbd_lo*theta, dtype=torch.float).to(self.device)) != y0 :
                lbd_lo = lbd_lo*0.99
                nquery += 1
        ...
        while (lbd_hi - lbd_lo) > tol:
            ...
            nquery += 1
            if self.predict(x0 + torch.tensor(lbd_mid*theta, dtype=torch.float).to(self.device)) != y0:
                lbd_hi = lbd_mid
            else:
                lbd_lo = lbd_mid
    ...
    def fine_grained_binary_search(self, x0, y0, theta, initial_lbd, current_best):
        ...
        if initial_lbd > current_best: 
            if self.predict(x0+torch.tensor(current_best*theta, dtype=torch.float).to(self.device)) == y0:
                nquery += 1
                return float('inf'), nquery
            lbd = current_best
        else:
